            ...
            },
            {
                ...
                options: [
                    ...
                    },
                    {
                        ...
                        type: 'string',
                        default: '',
                        description: 'Matches if the message or description contains this text (case-insensitive)',
                    },
                ],
            },
            {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            try {
                ...
                if (operation === 'filterEvents') {
                    ...
                    results = results.filter(item => applyHydratedFilters(item as IDataObject, {
                        eventTypes,
                        personIds,
                        eventSource,
                        propertyFilters,
...
function applyHydratedFilters(item: IDataObject, filters: {
    eventTypes: string[];
    personIds: string;
    eventSource: string;
    ...
    if (filters.eventSource) {
        if ((item.source as string || '').toLowerCase() !== filters.eventSource.toLowerCase()) return false;
    }
    ...
    if (pf && Object.keys(pf).length > 0) {
        ...
        if (pf.state && (prop.state as string || '').toLowerCase() !== (pf.state as string).toLowerCase()) return false;
        if (pf.city && (prop.city as string || '').toLowerCase() !== (pf.city as string).toLowerCase()) return false;

        if (pf.neighborhood) {
            const neighborhood = (prop.neighborhood as string || search.neighborhood as string || '').toLowerCase();
    ...
    if (ef && Object.keys(ef).length > 0) {
        ...
        if (ef.messageContains) {
            const text = `${item.message as string || ''} ${item.description as string || ''}`.toLowerCase();
    ...
    if (cf && Object.keys(cf).length > 0) {
        ...
        if (cf.source && !(camp.source as string || '').toLowerCase().includes((cf.source as string).toLowerCase())) return false;
        if (cf.medium && !(camp.medium as string || '').toLowerCase().includes((cf.medium as string).toLowerCase())) return false;
        if (cf.term && !(camp.term as string || '').toLowerCase().includes((cf.term as string).toLowerCase())) return false;
        if (cf.content && !(camp.content as string || '').toLowerCase().includes((cf.content as string).toLowerCase())) return false;
        if (cf.campaign && !(camp.campaign as string || '').toLowerCase().includes((cf.campaign as string).toLowerCase())) return false;
    }
