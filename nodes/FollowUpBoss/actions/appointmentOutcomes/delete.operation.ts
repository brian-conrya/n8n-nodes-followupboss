...
import { toInt, updateDisplayOptions, wrapDeleteSuccess, getAppointmentOutcomeIdProperty } from '../../helpers/utils';
...
const properties: INodeProperties[] = [
	...
	},
	{
		...
		description: 'The outcome ID to reassign existing appointments. Choose from the list, or specify an ID.',
	},
];
...
export async function execute(
	...
	const assignOutcomeIdRaw = (this.getNodeParameter('assignOutcomeId', i) as IDataObject).value as string;
	const assignOutcomeId = toInt(assignOutcomeIdRaw, 'Assign Outcome ID', this.getNode(), i);

	await apiRequest.call(this, 'DELETE', `/appointmentOutcomes/${id}`, undefined, { assignOutcomeId });
